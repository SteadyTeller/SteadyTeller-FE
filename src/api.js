...
export const api = {
  ...
  continueGoal: (goalId, payload) => request(`/api/v1/goals/${goalId}/deadline/continue`, { method: 'POST', body: JSON.stringify(payload) }),
  goals: () => request('/api/v1/goals'),
  createGoal: (payload) => request('/api/v1/goals', { method: 'POST', body: JSON.stringify(payload) }),
  generateGoalPlan: (payload) => request('/api/v1/goals/plan', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  deleteConfirmedTask: (taskId) => request(`/api/v1/confirmed-tasks/${taskId}`, { method: 'DELETE' }),
  ...
  deferToSupplement: (scheduleId, itemId) => request(`/api/v1/schedules/${scheduleId}/items/${itemId}/defer-to-supplement`, { method: 'PATCH' }),
  failScheduleItem: (scheduleId, itemId, payload) => request(`/api/v1/schedules/${scheduleId}/items/${itemId}/failure`, { method: 'PATCH', body: JSON.stringify(payload) }),
  replanRemaining: (scheduleId) => request(`/api/v1/schedules/${scheduleId}/replan-remaining`, { method: 'POST' }),
