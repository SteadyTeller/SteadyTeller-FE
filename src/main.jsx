...
function Dashboard({ name, goal, schedule, goalStatistics, onCreateGoal, onGoGoals, onGoSchedule, onStart, onComplete, onRevert }) {
  ...
  const progressRate = clampPercentage(goalStatistics?.progressRate)
  return <><section className="welcome-row"><div><div className="eyebrow"><span className="eyebrow-dot" /> CURRENT LEARNING GOAL</div><h1>안녕하세요, {name} 👋</h1><p>오늘도 목표를 향해 한 걸음 나아가볼까요?</p></div><button className="primary-button" onClick={onCreateGoal}><Plus size={18} />새 목표 만들기</button></section><section className="hero-grid"><div className="focus-card"><div className="focus-head"><div><span className="card-kicker">CURRENT GOAL</span><h2>{goal.title}</h2></div><div className="goal-icon"><Target size={26} /></div></div><div className="goal-meta"><span><CalendarDays size={15} />{goal.targetDate}</span><span><Clock3 size={15} />하루 {goal.dailyStudyHours}시간</span></div><div className="hero-progress"><div><span>목표 진행률</span><strong>{progressRate}%</strong></div><div className="progress-track"><span style={{ width: `${progressRate}%` }} /></div><small>{goalStatistics ? `${goalStatistics.completedSchedules} / ${goalStatistics.totalSchedules}개 학습 완료` : '학습 일정을 만들면 진행률이 표시됩니다.'}</small></div><button className="text-button" onClick={onGoGoals}>목표 자세히 보기 <ArrowRight size={16} /></button></div><div className="streak-card"><span className="card-kicker">SCHEDULE STATUS</span><h3>{schedule ? '생성된 학습 일정' : '학습 일정이 없어요'}</h3><div className="streak-number"><strong>{schedule?.dailySchedules?.length ?? 0}</strong><span>학습 예정<br />일자</span></div><p className="streak-tip">{schedule ? `${schedule.scheduleStartDate}부터 ${schedule.scheduleEndDate}까지` : '태스크를 확정한 뒤 일정을 만들어보세요.'}</p><button className="text-button purple" onClick={onGoSchedule}>일정 확인하기 <ArrowRight size={15} /></button></div></section><section className="today-card"><div className="today-card-head"><div><span className="card-kicker">TODAY'S PLAN · {goal.title}</span><h2>오늘 할 일</h2></div><span>{today}</span></div>{todayPlan?.items?.length ? <div className="today-items">{todayPlan.items.map(item => <div key={item.scheduleItemId}><span>{item.order}</span><strong>{item.title}</strong><small><Clock3 size={13} /> {item.allocatedMinutes}분</small><span className={`status ${item.status?.toLowerCase()}`}>{item.status}</span><ItemStatusAction item={item} onStart={onStart} onComplete={onComplete} onRevert={onRevert} /></div>)}</div> : <p className="today-empty">오늘 배정된 학습이 없어요. 다음 학습 일정을 확인해보세요.</p>}</section></>
}

function StatisticsPage({ goal, summary, daily, goalStatistics, loading }) {
  if (!goal) return <EmptyState title="학습 목표를 먼저 만들어주세요" description="목표와 스케줄이 생성되면 학습 통계를 확인할 수 있어요." />
  if (loading) return <div className="app-loading"><BarChart3 size={24} /><strong>학습 통계를 불러오는 중이에요.</strong></div>
  const progressRate = clampPercentage(goalStatistics?.progressRate)
  return <><section className="welcome-row"><div><div className="eyebrow"><span className="eyebrow-dot" /> LEARNING RECORDS</div><h1>학습 기록</h1><p>“{goal.title}”의 수행 기록과 계획 대비 완료 현황입니다.</p></div></section><section className="statistics-grid"><div className="stat-card stat-primary"><span>목표 진행률</span><strong>{progressRate}%</strong><div className="stat-progress"><i style={{ width: `${progressRate}%` }} /></div><small>{goalStatistics?.completedSchedules ?? 0} / {goalStatistics?.totalSchedules ?? 0}개 완료</small></div><div className="stat-card"><span>기간 완료율</span><strong>{clampPercentage(summary?.completionRate)}%</strong><small>{summary?.completedSchedules ?? 0} / {summary?.totalSchedules ?? 0}개 완료</small></div><div className="stat-card"><span>총 계획시간</span><strong>{formatMinutes(goalStatistics?.totalPlannedMinutes)}</strong><small>{goal.startDate} — {goal.targetDate}</small></div><div className="stat-card"><span>연속 학습일</span><strong>{summary?.consecutiveStudyDays ?? 0}일</strong><small>완료한 일정 기준</small></div></section><section className="daily-statistics"><div className="daily-statistics-head"><div><span className="card-kicker">DAILY PROGRESS</span><h2>날짜별 완료 현황</h2></div></div>{daily.length ? <div className="daily-statistics-list">{daily.map(day => <div key={day.studyDate}><time>{day.studyDate}</time><div className="daily-progress"><i style={{ width: `${clampPercentage(day.completionRate)}%` }} /></div><strong>{clampPercentage(day.completionRate)}%</strong><span>{day.completedCount}/{day.scheduleCount}개 · {formatMinutes(day.plannedMinutes)}</span></div>)}</div> : <p className="statistics-empty">아직 집계할 학습 일정이 없어요.</p>}</section></>
}

function MyInfoPage({ member }) {
  return <><section className="welcome-row"><div><div className="eyebrow"><span className="eyebrow-dot" /> ACCOUNT & SETTINGS</div><h1>내 정보 관리</h1><p>회원 정보와 계정 보안 설정을 관리하는 영역입니다.</p></div></section><section className="empty-state"><h2>{member?.nickname ?? '회원'}님의 계정</h2><p>프로필, 비밀번호 변경, 로그아웃 등의 기능을 이 페이지에서 관리합니다.</p></section></>
}

function GoalsPage({ goals, selectedGoal, tasks, schedule, onSelect, onCreate, onEdit, onDelete, onDeleteTask, onConfirmTasks, onGenerateTasks, onGenerateSchedule, onDeleteSchedule, isGeneratingTasks, isGeneratingSchedule, onStart, onComplete, onRevert }) {
...
function App() {
  ...
  const loadGoalData = useCallback(async goalId => { if (!goalId) { confirmedTasksForDisplay = []; setTasks([]); setSchedule(null); return }; const [candidates, confirmed, summaries] = await Promise.all([api.candidates(goalId), api.confirmedTasks(goalId), api.schedules(goalId)]); confirmedTasksForDisplay = confirmed ?? []; setTasks(candidates ?? []); const latest = summaries?.[0]; setSchedule(latest ? await api.schedule(latest.scheduleId) : null) }, [])
  const loadStatistics = useCallback(async goal => { if (!goal) { setStatistics({ summary: null, daily: [], goal: null }); return }; const range = statisticsDateRange(goal); if (!range) return; setStatisticsLoading(true); try { const [summary, daily, goalResult] = await Promise.all([api.statisticsSummary(range.startDate, range.endDate), api.dailyStatistics(range.startDate, range.endDate), api.goalStatistics(goal.id)]); setStatistics({ summary, daily: daily ?? [], goal: goalResult }) } finally { setStatisticsLoading(false) } }, [])
  const refresh = useCallback(async () => { setLoading(true); setError(''); try { const [me, loadedGoals] = await Promise.all([api.member(), api.goals()]); setMember(me); setGoals(loadedGoals ?? []); const nextGoal = loadedGoals?.find(goal => goal.id === selectedGoalId) ?? loadedGoals?.[0] ?? null; setSelectedGoalId(nextGoal?.id ?? null); await Promise.all([loadGoalData(nextGoal?.id), loadStatistics(nextGoal)]) } catch (err) { setError(err.message) } finally { setLoading(false) } }, [loadGoalData, loadStatistics, selectedGoalId])
  useEffect(() => { if (token) refresh() }, [token])
  useEffect(() => { if (!token || !selectedGoal) return; Promise.all([loadGoalData(selectedGoal.id), loadStatistics(selectedGoal)]).catch(err => setError(err.message)) }, [token, selectedGoalId])
  useEffect(() => { const handleAuthExpired = () => { setToken(null); setMember(null); setGoals([]); setTasks([]); setSchedule(null); setStatistics({ summary: null, daily: [], goal: null }) }; window.addEventListener(AUTH_EXPIRED_EVENT, handleAuthExpired); return () => window.removeEventListener(AUTH_EXPIRED_EVENT, handleAuthExpired) }, [])
  const logout = () => { localStorage.removeItem('steadyTeller.accessToken'); setToken(null); setMember(null); setGoals([]); setTasks([]); setSchedule(null); setStatistics({ summary: null, daily: [], goal: null }) }
  const saveGoal = async form => { try { const saved = goalModal?.id ? await api.updateGoal(goalModal.id, form) : await api.createGoal(form); setGoals(current => goalModal?.id ? current.map(goal => goal.id === saved.id ? saved : goal) : [...current, saved]); setSelectedGoalId(saved.id); await loadStatistics(saved); showToast('학습 목표가 저장됐어요.') } catch (err) { showToast(err.message); throw err } }
  ...
  const confirmTasks = async () => { if (!selectedGoal) return; try { await api.confirmTasks(selectedGoal.id); confirmedTasksForDisplay = await api.confirmedTasks(selectedGoal.id); setTasks([]); showToast('태스크가 확정됐어요. 이제 일정을 생성할 수 있어요.') } catch (err) { showToast(err.message) } }
